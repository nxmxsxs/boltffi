...
This compiles a cdylib for the host platform, generates Java bindings and a C header, compiles the JNI glue into a shared library, and writes everything to `dist/java/`.
...
│   └── mylib.h             # C header
└── libmylib_jni.dylib      # Compiled JNI library (.so on Linux)
```

### Using in a Project

1. Copy the Java sources from `dist/java/com/...` into your source tree
2. Place the JNI shared library where `System.loadLibrary` can find it (or set `java.library.path`)
