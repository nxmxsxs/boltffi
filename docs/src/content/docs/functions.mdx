...
Mark a function `async` and BoltFFI generates an async function in the target language. Swift gets `async`, Kotlin gets `suspend`, Java gets `CompletableFuture` (Java 8+) or virtual-thread blocking calls (Java 21+), and TypeScript gets `Promise`.
