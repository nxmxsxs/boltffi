...
3. Run `boltffi pack apple`, `boltffi pack android`, `boltffi pack java`, or `boltffi pack wasm`. To generate all of them at once, run `boltffi pack all`
4. Import the generated framework in your project

The generated bindings use native idioms. Swift gets `async`/`await` and throwing functions. Kotlin gets coroutines and sealed classes. Java gets CompletableFuture and records (Java 16+) or plain classes (Java 8+). TypeScript gets Promises and typed interfaces. Your Rust library feels like a native SDK.
...
BoltFFI has full support for Swift, Kotlin, Java, and TypeScript (WASM). All features documented in this manual work for all four languages. Adding support for more languages is in progress.
