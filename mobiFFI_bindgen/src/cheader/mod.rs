...
use mobiFFI_ffi_rules::{c_types, naming::{self, snake_to_camel}, signatures::FfiParam};
...
impl CHeaderGenerator {
    ...
    fn function_signatures(func: &Function, prefix: &str) -> Vec<FunctionView> {
        ...
        match &func.output {
            Some(Type::Vec(inner)) => {
                ...
                let sigs = mobiFFI_ffi_rules::signatures::vec_return_signatures(
                    &ffi_name,
                    &inner_c,
                    &input_params,
                );
            ...
            Some(Type::String) => {
                let sig =
                    mobiFFI_ffi_rules::signatures::string_return_signature(&ffi_name, &input_params);
                vec![FunctionView {
    ...
    fn param_to_ffi(param: &Parameter) -> Vec<FfiParam> {
        match &param.param_type {
            Type::String => mobiFFI_ffi_rules::signatures::string_param(&param.name),
            Type::Vec(inner) => {
                let inner_c = Self::type_to_c(inner);
                mobiFFI_ffi_rules::signatures::vec_param(&param.name, &inner_c)
            }
            Type::Slice(inner) => {
                let inner_c = Self::type_to_c(inner);
                mobiFFI_ffi_rules::signatures::slice_param(&param.name, &inner_c, false)
            }
            Type::MutSlice(inner) => {
                let inner_c = Self::type_to_c(inner);
                mobiFFI_ffi_rules::signatures::slice_param(&param.name, &inner_c, true)
            }
            _ => vec![FfiParam {
